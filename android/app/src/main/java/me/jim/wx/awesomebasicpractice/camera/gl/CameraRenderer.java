...
import me.jim.wx.awesomebasicpractice.camera.ShaderUtils;
...
public class CameraRenderer extends Thread implements SurfaceTexture.OnFrameAvailableListener
{
    ...
    protected float mPreviewWHRatio;
    ...
    private static short drawOrder[] = {0, 1, 3, 1, 2, 3};
     ...
     */
    public CameraRenderer(Context context, SurfaceTexture texture, TextureView textureView,
                          int width, int height)
    {
        init(context, texture, textureView, width, height,
                DEFAULT_FRAGMENT_SHADER, DEFAULT_VERTEX_SHADER,true);
    }
     ...
     */
    public CameraRenderer(Context context, SurfaceTexture texture, TextureView textureView,
                          int width, int height, String fragPath, String vertPath, boolean isLoadShaderFromAssest)
    {
        init(context, texture,textureView, width, height, fragPath, vertPath,isLoadShaderFromAssest);
    }

    private void init(Context context, SurfaceTexture texture, TextureView textureView,
                      int width, int height, String fragPath, String vertPath, boolean isLoadShaderFromAssest)
    {
        ...
        if(isLoadShaderFromAssest){
            this.mFragmentShaderPath = fragPath;
            this.mVertexShaderPath = vertPath;
        }else{
            this.fragmentShaderCode = fragPath;
            this.vertexShaderCode = vertPath;
        }
    ...
    private void initialize() {
        ...
        if((TextUtils.isEmpty(fragmentShaderCode) || TextUtils.isEmpty(vertexShaderCode))) {
    ...
    public void initGL() {
        ...
        initGLComponents();
        Log.d(TAG,"initGL thread name:"+ Thread.currentThread().getName());
    }
    ...
    public  void setPreivewWHRatio(final float previewWHRatio){
        mPreviewWHRatio = previewWHRatio;
    }
    ...
    protected void setUniformsAndAttribs()
    {
        ...
        int previewWHRatioLoc = GLES20.glGetUniformLocation(mCameraShaderProgram,"previewWH");

        if(mPreviewWHRatio < 1){
            GLES20.glUniform1f(previewWHRatioLoc, mPreviewWHRatio);
        }else {
            GLES20.glUniform1f(previewWHRatioLoc, 1);
        }
